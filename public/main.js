...
(() => {
  ...
  new Vue({
    ...
    methods: {
      ...
      //     this.activeTimers = activeTimers; //todo надо использовать
      //   });
      // },
      // fetchOldTimers() {
      //   fetchJson("/api/timers?isActive=false").then((oldTimers) => {
      //     this.oldTimers = oldTimers;// todo надо использвать
      //   });
      // },
      createTimer() {
      ...
      formatTime(ts) {
        return new Date(Number(ts)).toTimeString().split(" ")[0]; // перевод ts в Numbr(ts), т.к. приходит строка
      },
      formatDuration(d) {
        d = Math.floor(d / 1000);
        const s = d % 60;
